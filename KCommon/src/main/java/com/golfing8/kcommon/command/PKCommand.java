...
public abstract class PKCommand implements TabExecutor {
     ...
     */
    @Getter
    private final List<PKCommand> subcommands = new ArrayList<>();
     ...
     */
    protected void setParent(@Nullable PKCommand parent) {
        this.parent = parent;
    }
    ...
    protected final void subRegister() {
        for(PKCommand sub : this.subcommands) {
            sub.onRegister();
            sub.subRegister();
        }
    }
     ...
     */
    protected final void addSubCommand(PKCommand command) {
        if(command == this)
    ...
    private boolean checkSubcommands(CommandSender sender, String[] args) {
        ...
        for(PKCommand subcommand : this.subcommands) {
    ...
    public final List<String> gatherTabCompletions(CommandSender sender, String label, String[] args) {
        ...
        for (PKCommand command : this.getSubcommands()) {
